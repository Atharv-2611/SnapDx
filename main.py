...
def login():
    if request.method == 'POST':
        ...
        if user and check_password_hash(user['password'], password):
            ...
            print(f"Login successful - email: {email}, role: {role}, session: {dict(session)}")
            if role == 'doctor':
...
def paitent_chatbot():
    print(f"Patient chatbot route - Session data: {dict(session)}")
    if 'email' not in session or session.get('role') != 'patient':
        print(f"Authentication failed - email: {session.get('email')}, role: {session.get('role')}")
        return redirect(url_for('login'))
    return render_template("paitent/chatbot.html", user_email=session.get('email'))

@app.route("/patient/chat")
def paitent_chat():
    print(f"Patient chat route - Session data: {dict(session)}")
    if 'email' not in session or session.get('role') != 'patient':
        print(f"Authentication failed - email: {session.get('email')}, role: {session.get('role')}")
        return redirect(url_for('login'))
...
def get_report_by_id(report_id):
    ...
    try:
        ...
        if not diagnosis:
            print(f"Report ID {report_id} not found in database")
            # Let's also check what report IDs exist for debugging
            all_reports = list(diagnoses_collection.find({}, {'report_id': 1}))
            print(f"Available report IDs in database: {[r.get('report_id') for r in all_reports]}")
...
def list_available_reports():
    """List all available report IDs for testing"""
    try:
        reports = list(diagnoses_collection.find({}, {'report_id': 1, 'disease_name': 1, 'created_at': 1}))
        report_list = []
...
def get_doctor_stats():
    """Get live statistics for the logged-in doctor"""
    if 'email' not in session or session.get('role') != 'doctor':
        return jsonify({'success': False, 'error': 'Unauthorized'}), 401
    ...
    try:
        ...
        total_patients = diagnoses_collection.distinct('patient_email', {'doctor_email': doctor_email})
        total_patients_count = len(total_patients)
        
        # If no patients found by email, try by patient_id
        if total_patients_count == 0:
            total_patients = diagnoses_collection.distinct('patient_id', {'doctor_email': doctor_email})
            total_patients_count = len(total_patients)
        
        # Count diagnoses completed today
        completed_today = diagnoses_collection.count_documents({
            'doctor_email': doctor_email,
            'created_at': {
                '$gte': today,
                '$lt': tomorrow
